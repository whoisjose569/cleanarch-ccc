...
def test_handle_by_value():
    ...
    response = paid_bills_finder_controller.handle_by_value(http_request)
    
    assert isinstance(response, HttpResponse)
    assert response.status_code == 200
    assert response.body['data'] is not None

def test_handle_by_payment_date():
    http_request_mock = HttpRequestMock()
    use_case = PaidBillsFinderSpy()
    paid_bills_finder_controller = PaidBillsFinderController(use_case)
    http_request = HttpRequest()
    ...
    response = paid_bills_finder_controller.handle_by_payment_date(http_request)
    
    assert isinstance(response, HttpResponse)
    assert response.status_code == 200
    assert response.body['data'] is not None

def test_handle_by_type():
    http_request_mock = HttpRequestMock()
    use_case = PaidBillsFinderSpy()
    paid_bills_finder_controller = PaidBillsFinderController(use_case)
    http_request = HttpRequest()
